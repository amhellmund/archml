...
class ArchmlVisualizeDirective(Directive):
    ...
    Options:
        ...
        image-width: CSS width for the rendered image (e.g. ``400px``).
            Optional.
        variant: Filter the diagram to a specific variant.  Use ``all`` to
            show every element regardless of variant annotation (default),
            ``baseline`` to show only baseline elements (those with no
            variant annotation), or any user-defined variant name to show
            baseline elements plus elements annotated with that variant.
            Optional.
    """
    ...
    def run(self) -> list[nodes.Node]:
        ...
        variant_opt: str | None = self.options.get("variant")
        # "all" means no filtering; omitting the option also means no filtering.
        variant: str | None = None if (variant_opt is None or variant_opt == "all") else variant_opt

        try:
            svg_path = _generate_diagram(env, root_entity, depth, variant=variant)
...
def _generate_diagram(env: Any, root_entity: str, depth: int | None, *, variant: str | None = None) -> Path:
    ...
    Args:
        ...
        variant: Variant filter passed to the topology builder.  ``None``
            shows all elements; ``"baseline"`` shows only baseline elements;
            any other string filters to baseline plus that variant.
    ...
    viz_diagram = build_viz_diagram(entity, depth=depth, variant=variant)
    ...
    variant_suffix = f"_v{_sanitize_name(variant)}" if variant is not None else ""
    img_name = f"{_sanitize_name(root_entity)}{depth_suffix}{variant_suffix}.svg"
