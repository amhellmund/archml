...
def test_load_config_with_local_path_import(tmp_path):
    ...
    cfg_file = tmp_path / ".farchml-workspace.yaml"
    cfg_file.write_text(
...
def test_load_config_default_remote_sync_directory(tmp_path):
    """When remote-sync-directory is absent, the default is '.farchml-remotes'."""
    cfg_file = tmp_path / ".farchml-workspace.yaml"
    cfg_file.write_text(
        "name: myworkspace\nbuild-directory: build\nsource-imports:\n  - name: src\n    local-path: .\n",
        encoding="utf-8",
    ...
    assert config.remote_sync_directory == ".farchml-remotes"
...
def test_valid_mnemonic_names(tmp_path):
    ...
    for name in valid_names:
        cfg_file = tmp_path / f".farchml-workspace-{name}.yaml"
        cfg_file.write_text(
            f"name: myworkspace\nbuild-directory: build\nsource-imports:\n  - name: {name}\n    local-path: .\n",
            encoding="utf-8",
        )
